...
- **问题**：固定选 Read/Bash 做示例，与用户当前操作脱节，影响工具调用命中率
- **新增 `getLastUsedToolName()`**：从对话历史中从后往前扫描 assistant 消息，提取最近一次 `tool_use` block 的工具名
- 优先用历史中最近用到的工具做 few-shot 示例，无历史时回退到 Read/Bash 默认示例
- 改动集中在 `converter.ts`，不影响任何核心逻辑

### 🧠 多维度打分拒绝检测

- **问题**：单一正则匹配，新措辞变体（如 `As a coding assistant, I'm here to help with coding tasks only`）无法命中，导致漏检
- **重构 `isRefusal()`**：三级权重打分（高 +3 / 中 +2 / 低 +1），多特征叠加超过阈值（≥3）才判定为拒绝
  - 高权重：身份声明、工具锁定、注入检测等强信号
  - 中权重：话题限制、能力边界等限制性信号
  - 低权重：弱信号，单独不触发，组合叠加
- 短响应动态加分（< 150 字符 +1），拒绝通常简短
- 精确限定主语（`I cannot perform` 而非 `cannot perform`），避免误判技术描述
- **召回率 100%**（28/28）、**误报率 0%**（22/22），经 50 条样本验证
- 新增环境变量 `LOG_REFUSAL_FEATURES=true` 开启命中特征结构化日志，便于上线后调优

### 🔗 语义锚点续写

- **问题**：Tier 3/4 续写锚点取末尾 300 字符，可能落在代码块/XML 中间，导致模型从错误位置续写
- **新增 `extractSemanticAnchor()`**：优先回溯到最近段落边界（空行），其次行首，最后才兜底取末尾字符
- 续写锚点始终从完整语义单元开始，提高续写成功率

### 📢 输出预算预警提示词

- 在 `behaviorRules` 中加入输出预算提示：告知模型输出预算有限，主动将大文件/长内容拆分为 ≤150 行的多次 Write 调用
- 作为截断的预防层，配合现有阶梯续写形成「预防 + 兜底」双保险
